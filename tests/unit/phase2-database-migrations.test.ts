...
describe("phase 2 database migrations", () => {
  it("creates all v1 tables from a clean sqlite database", () => {
    ...
    expect(tables).toEqual([
      ...
      "chapters",
      "import_jobs",
      "outline_chapter_notes",
      "outline_event_threads",
      "outline_events",
      "outline_threads",
      "projects",
      "prompt_presets",
  ...
  it("can run migrations repeatedly without duplicating versions", () => {
    ...
    expect(db.prepare("SELECT COUNT(*) AS count FROM schema_migrations").get()).toEqual({ count: 22 });
  ...
  it("repairs missing usage analytics tables when an older prototype marked v22 applied", () => {
    ...
    expect(db.prepare("SELECT version FROM schema_migrations ORDER BY version DESC LIMIT 1").get()).toEqual({ version: 25 });
