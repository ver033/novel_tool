...
function sourceStatusFromAuthorData(
  ...
  return {
    ...
    message: hasData
      ? `作者手工人物关系已就绪：${characters.length} 个人物，${relationships.length} 条关系。`
      : "作者还没有录入人物关系。",
    latestFailure: null
  };
}
...
export class AuthorRelationshipGraphService {
  ...
  getGraph(input: RelationshipGraphGetInput, generatedAt = new Date().toISOString()): RelationshipGraphResult {
    ...
    const edges = queryRelationships
      ...
      .map((relationship) => this.toGraphEdge(relationship, charactersById))
    ...
    return {
      ...
      graphStats: {
        totalMentionCount: relationships.length,
        usedMentionCount: edges.length,
        hiddenMentionCount: Math.max(0, relationships.length - edges.length),
  ...
  private toGraphEdge(
    ...
    return {
      ...
      semanticMarkers: [AUTHOR_DIMENSION.name, relationship.sourceToTargetLabel, relationship.targetToSourceLabel].filter(
